...
pub fn run<I: BufRead, O: Write + Send>(input: I, output: O, config: Config) -> Result<()> {
    ...
    // The renderer's cell pixel size lets the terminal track which cells later
    // content draws over each sixel image, so the renderer can occlude them.
    ...
    let frames: Vec<frames::Frame> = match plan {
        ...
        selection::SelectionPlan::Range { start, end } => {
            let frames = frames::from_range(&events, terminal_size, cell_size, start, end);
        ...
        selection::SelectionPlan::Positions(positions) => {
            let frames = frames::at_positions(&events, terminal_size, cell_size, positions);
